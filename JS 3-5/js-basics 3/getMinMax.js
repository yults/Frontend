...
export default function getMinMax(input) {
  if (input.search(/\d+|Infinity/) < 0) {
    return {
      undefined
    };
  }
  const regex = /[-+]?[0-9]+(\.[0-9]+)?|-?Infinity/g;
  const parse = input.match(regex).map(Number);

  return {
    min: Math.min.apply(null, parse),
    max: Math.max.apply(null, parse)
  };
}
